#!/bin/bash
# Script pour créer les fichiers .env pour backend, web et docker_compose

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKEND_ENV="${SCRIPT_DIR}/backend/.env"
WEB_ENV="${SCRIPT_DIR}/web/.env"
DOCKER_COMPOSE_ENV="${SCRIPT_DIR}/deployment/docker_compose/.env"
...
# Créer web/.env
echo "✅ Création de web/.env"
cat > "${WEB_ENV}" << EOF
ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true
EOF

# Créer deployment/docker_compose/.env
echo "✅ Création de deployment/docker_compose/.env"
cat > "${DOCKER_COMPOSE_ENV}" << 'ENVEOF'
...
cat "${BACKEND_ENV}"
echo ""
echo "📋 Contenu de web/.env:"
cat "${WEB_ENV}"
echo ""
echo "📋 Fichier deployment/docker_compose/.env créé (${#DOCKER_COMPOSE_ENV} caractères)"
